...
def train_and_get_data_RBME(dataset, n_vis, n_hid, k, n_epochs, batch_size, lr, momentum, filename):
    ...
    for epoch in range(n_epochs):
        ...
        print('Epoch %3.d | Loss=%6.4f' % (epoch+1, np.mean(loss_), ))
    ...
    Hs=get_H_s(a,b)
    Hk=get_H_k(a,b)
    with open('models/%s_model_n_hid=%s_%s.pkl' %(str(datetime.today())[:10], n_hid, filename), 'wb') as f:
        pkl.dump(model, f)
    with open('data/%s_data_n_hid=%s_%s.pkl' %(str(datetime.today())[:10], n_hid, filename), 'wb') as f:
